...
#define SHOW_MODE_WAVES VideoState::ShowMode::SHOW_MODE_WAVES
...
static enum VideoState::ShowMode show_mode = SHOW_MODE_NONE;
...
static void video_image_display(VideoState *is)
{
    ...
    if (!vp->uploaded) {
        if (my_upload_texture(&is->vid_texture, vp->frame, &is->img_convert_ctx) < 0)
            return;
        vp->uploaded = 1;
